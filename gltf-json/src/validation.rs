...
pub trait Validate {
    ...
    fn validate<P, R, E: crate::CustomExtensions>(&self, _root: &Root<E>, _path: P, _report: &mut R)
    where
        P: Fn() -> Path,
...
impl<T> Validate for Checked<T> {
    fn validate<P, R, E: crate::CustomExtensions>(&self, _root: &Root<E>, path: P, report: &mut R)
    where
        P: Fn() -> Path,
        R: FnMut(&dyn Fn() -> Path, Error),
    {
...
impl Validate for std::boxed::Box<serde_json::value::RawValue> {
    fn validate<P, R, E: crate::CustomExtensions>(&self, _: &Root<E>, _: P, _: &mut R)
    where
        P: Fn() -> Path,
        R: FnMut(&dyn Fn() -> Path, Error),
    {
