...
impl<'a> Material<'a> {
    ...
    pub(crate) fn new(doc: &'a Document, index: usize, json: &'a json::material::Material) -> Self {
        Self {
            doc,
            index: Some(index),
            json: json,
    ...
    pub fn pbr_metallic_roughness(&self) -> PbrMetallicRoughness<'a> {
        PbrMetallicRoughness::new(self.doc, &self.json.pbr_metallic_roughness)
    }
    ...
    pub fn normal_texture(&self) -> Option<NormalTexture<'a>> {
        self.json.normal_texture.as_ref().map(|json| {
            let texture = self.doc.textures().nth(json.index.value()).unwrap();
...
impl<'a> PbrMetallicRoughness<'a> {
    ...
    pub(crate) fn new(doc: &'a Document, json: &'a json::material::PbrMetallicRoughness) -> Self {
        Self {
            doc,
            json: json,
        }
