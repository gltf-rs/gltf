...
use {Accessor, Document, Material};
...
impl<'a> Mesh<'a> {
    /// Constructs a `Mesh`.
    pub(crate) fn new(doc: &'a Document, index: usize, json: &'a json::mesh::Mesh) -> Self {
        Self {
            doc,
            index: index,
            json: json,
...
impl<'a> Primitive<'a> {
    ...
    pub fn position_bounds(&self) -> Option<Bounds<[f32; 3]>> {
        if let Some(pos_accessor_index) = self.json
            .attributes
    ...
    pub fn get(&self, semantic: &Semantic) -> Option<Accessor> {
        self.json
            ...
            .map(|index| self.mesh.doc.accessors().nth(index.value()).unwrap())
    }
    ...
    pub fn material(&self) -> Material {
        self.json
            .material
            .as_ref()
            .map(|index| self.mesh.doc.materials().nth(index.value()).unwrap())
            .unwrap_or_else(|| Material::default(self.mesh.doc))
    }
...
impl<'a> Iterator for Attributes<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.iter.next().map(|(key, index)| {
            ...
            let accessor = self.doc.accessors().nth(index.value()).unwrap();
            (semantic, accessor)
        })
    }
