...
impl<'a> Sampler<'a> {
    ...
    pub(crate) fn new(doc: &'a Document, index: usize, json: &'a json::texture::Sampler) -> Self {
        Self {
            doc,
            index: Some(index),
            json: json,
    ...
    pub(crate) fn default(doc: &'a Document) -> Self {
        Self {
            doc,
            index: None,
...
impl<'a> Texture<'a> {
    ...
    pub(crate) fn new(doc: &'a Document, index: usize, json: &'a json::texture::Texture) -> Self {
        Self {
            doc,
            index: index,
            json: json,
    ...
    pub fn sampler(&self) -> Sampler<'a> {
        self.json
            .sampler
            .as_ref()
            .map(|index| self.doc.samplers().nth(index.value() as usize).unwrap())
            .unwrap_or_else(|| Sampler::default(self.doc))
    }
...
impl<'a> Info<'a> {
    ...
    pub(crate) fn as_json(&self) -> &json::texture::Info {
        self.json
    }
