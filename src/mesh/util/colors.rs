...
use mesh::util::IterColors;

/// Casting iterator for `Colors`.
#[derive(Clone, Debug)]
pub struct CastingIter<'a, T>(IterColors<'a>, PhantomData<T>);
...
impl<'a, A> CastingIter<'a, A> {
    pub(crate) fn new(iter: IterColors<'a>) -> Self {
        CastingIter(iter, PhantomData)
    }

    /// Unwrap underlying `IterColors` object.
    pub fn unwrap(self) -> IterColors<'a> {
        self.0
    }
}
...
impl<'a, A: Cast> Iterator for CastingIter<'a, A> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.0 {
            IterColors::RgbU8(ref mut i)   => i.next().map(A::cast_rgb_u8),
            IterColors::RgbU16(ref mut i)  => i.next().map(A::cast_rgb_u16),
            IterColors::RgbF32(ref mut i)  => i.next().map(A::cast_rgb_f32),
            IterColors::RgbaU8(ref mut i)  => i.next().map(A::cast_rgba_u8),
            IterColors::RgbaU16(ref mut i) => i.next().map(A::cast_rgba_u16),
            IterColors::RgbaF32(ref mut i) => i.next().map(A::cast_rgba_f32),
        }
    }
    ...
    fn nth(&mut self, x: usize) -> Option<Self::Item> {
        match self.0 {
            IterColors::RgbU8(ref mut i)   => i.nth(x).map(A::cast_rgb_u8),
            IterColors::RgbU16(ref mut i)  => i.nth(x).map(A::cast_rgb_u16),
            IterColors::RgbF32(ref mut i)  => i.nth(x).map(A::cast_rgb_f32),
            IterColors::RgbaU8(ref mut i)  => i.nth(x).map(A::cast_rgba_u8),
            IterColors::RgbaU16(ref mut i) => i.nth(x).map(A::cast_rgba_u16),
            IterColors::RgbaF32(ref mut i) => i.nth(x).map(A::cast_rgba_f32),
        }
    }

    fn last(self) -> Option<Self::Item> {
        match self.0 {
            IterColors::RgbU8(i)   => i.last().map(A::cast_rgb_u8),
            IterColors::RgbU16(i)  => i.last().map(A::cast_rgb_u16),
            IterColors::RgbF32(i)  => i.last().map(A::cast_rgb_f32),
            IterColors::RgbaU8(i)  => i.last().map(A::cast_rgba_u8),
            IterColors::RgbaU16(i) => i.last().map(A::cast_rgba_u16),
            IterColors::RgbaF32(i) => i.last().map(A::cast_rgba_f32),
        }
    }
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        match self.0 {
            IterColors::RgbU8(ref i)   => i.size_hint(),
            IterColors::RgbU16(ref i)  => i.size_hint(),
            IterColors::RgbF32(ref i)  => i.size_hint(),
            IterColors::RgbaU8(ref i)  => i.size_hint(),
            IterColors::RgbaU16(ref i) => i.size_hint(),
            IterColors::RgbaF32(ref i) => i.size_hint(),
        }
    }
}
